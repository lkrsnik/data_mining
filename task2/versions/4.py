...
def calculate_model(X, y, alpha):
    lr = Orange.regression.LassoRegressionLearner(alpha = alpha) #linearna regresija L2 - lr je objekt, ki se ucije metoda, ki pricakuje podatke
...
if (not(cross_validation)):
    ...
    for j in range(5):
        i = 0
        low_limit = int((len(train_set) * j) / 5)
        high_limit = int((len(train_set) * (j + 1)) / 5)
