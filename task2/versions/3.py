...
def magic(descriptors, train_set, test_set):
    ...
    intensity_model = calculate_model(intensity_X, intensity_Y)
    intensity_result = intensity_model(test_X)

    # calculates others
    rest_result = []
    for i in range(rest_Y.shape[1]):
        rest_model = calculate_model(rest_X, rest_Y[:,i])
...
if (not(cross_validation)):
    results = magic(descriptors, train_set, test_set)
    IO.exportResults("predict.txt", "result", legend, results)
else:
    i = 0
    limit = int((len(train_set) * 4) / 5)
    ...
    print (magical_test_set)
    print (type(test_set))
    print (type(magical_test_set))
    results = magic(descriptors, new_train_set, magical_test_set)
