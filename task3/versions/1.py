...
class PFit:
    def __init__(self, learner, ps):
        self.l = learner
        self.ps = ps
    def __call__(self, intensity_X, intensity_Y, rest_X, rest_Y):
        for p in self.ps:
            final = 0
            intensity_result_F = []
            rest_result_F = []
            for j in range(k_in):
                i = 0
                low_limit_intensity = int((intensity_X.shape[0] * j) / k_in)
                high_limit_intensity = int((intensity_X.shape[0] * (j + 1)) / k_in)
                low_limit_rest = int((rest_X.shape[0] * j) / k_in)
                high_limit_rest = int((rest_X.shape[0] * (j + 1)) / k_in)
                ...
                new_rest_X = (rest_X.T[:, np.concatenate((np.arange(0, low_limit_rest), np.arange(high_limit_rest, rest_X.shape[0])))]).T
                new_rest_Y = (rest_Y.T[:, np.concatenate((np.arange(0, low_limit_rest), np.arange(high_limit_rest, rest_Y.shape[0])))]).T

                new_intensity_test_X = (intensity_X.T[:, range(low_limit_intensity, high_limit_intensity)]).T
                new_rest_test_X = (rest_X.T[:, range(low_limit_rest, high_limit_rest)]).T

                # calculates intensity
                intensity_model = calculate_model(self.l, new_intensity_X, new_intensity_Y, p)
                intensity_result = intensity_model(new_intensity_test_X)
                intensity_result_F = np.concatenate((intensity_result_F, intensity_result))

                # calculates others
                rest_result = []
                for i in range(rest_Y.shape[1]):
                    rest_model = calculate_model(self.l, new_rest_X, new_rest_Y[:,i], p)
                    rest_result = np.concatenate((rest_result, rest_model(new_rest_test_X)))
                rest_result = rest_result.reshape((len(rest_result)/len(intensity_result)),len(intensity_result))
                rest_result_F = np.concatenate((rest_result_F, rest_result))
                # stacks and export results
...
intensity_X, intensity_Y, rest_X, rest_Y = PipeData.create_train_set(train_set, descriptors)
bestscenario = PFit(Orange.regression.linear.LassoRegressionLearner, [0.8])
bestscenario(intensity_X, intensity_Y, rest_X, rest_Y)
print (score(legend))
