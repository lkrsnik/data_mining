...
def calculate_model(f, X, y, alpha):
    ...
    lr = f(alpha = alpha) #linearna regresija L2 - lr je objekt, ki se ucije metoda, ki pricakuje podatke
...
def preprocess_cross_validation(j, intensity_X, intensity_Y, intensity_dulition, k, localized):
    ...
    if not localized:
        print ("J = 0")
        print (new_intensity_test_X)
        print (intensity_dulition)
        print ("J = 0 -> END")

    #if j == 4:
        #print (intensity_X.shape, intensity_Y.shape, intensity_dulition.shape)
        #print (intensity_X.T[:, range(low_limit_intensity, high_limit_intensity)].shape)
        #print (intensity_X.T[:, np.arange(low_limit_intensity, high_limit_intensity)].shape)
        #print (high_limit_intensity)
        #print (high_limit_intensity - low_limit_intensity)
        
        #print (intensity_X.T[:, 105])
        #print (new_intensity_test_X.shape)
        #print (intensity_dulition[0])
        #print (np.arange(low_limit_intensity, high_limit_intensity))
        #print (intensity_X)
        #print (intensity_dulition)
    if localized:
        i = low_limit_intensity
        new_intensity_dulition = []
        print (len(intensity_dulition))
    ...
    if not localized:
        print ("J = 0 EEEEEEEE")
        print (new_intensity_X)
        print (new_intensity_dulition)
        print (new_intensity_X.shape)
        print (len(new_intensity_dulition))
        print (intensity_dulition[low_limit_intensity - 1])
        print ("J = 0 -> END EEEEEEEE")

    #if j == 1:
    #    return
    #print (len(new_intensity_dulition))
    #new_intensity_dulition = intensity_dulition[np.arange(low_limit_intensity, high_limit_intensity)]
    #print (new_intensity_X.shape, new_intensity_Y.shape, new_intensity_dulition.shape)
    ...
    for j in range(k):
        ...
        if intensity:
            if localized:
                ...
                print (intensity_X.shape, intensity_Y.shape, intensity_dulition.shape)
                intensity_model = f(new_intensity_X, new_intensity_Y, new_intensity_dulition)
    ...
    def __call__(self, intensity_X, intensity_Y, intensity_dulition, intensity = True):
        ...
        for p in self.ps:
            intensity_result_F = custom_cross_validation(self.l, p, intensity_X, intensity_Y, intensity_dulition, k_in, intensity, True)
                #print ("NOT NEEDED")
                #IO.exportResults("output", new_intensity_dulition, legend, intensity_result)
                #final += score(legend)
                #print ("STOP!")
            #if final / k_in > best_score:
            IO.exportResults("output", intensity_dulition, legend, intensity_result_F)
            fscore = score(legend)
            if fscore > best_score:
                #best_score = final / k_in
                best_score = fscore
                best_p = p

        print ("ALPHA =", best_p)
        print ("THE FINAL", best_score)
...
#print (intensity_X)
#print (intensity_dulition)
custom_cross_validation(lassoPreprocess, alphas, intensity_X, intensity_Y, intensity_dulition, k_out, True, False)
print ("Over!")
