...
def main():
    ...
    changing_theta = np.zeros(len (indexes) + 1).T
    ...
    # -------------------------- CV ---------------------------------
        ...
        # ---------------------------------------------------------------
        X = calculateX(X_train, indexes)
        #theta1 = learnTheta(X, intensity)
        theta = learnTheta(X, y_train, CustomFunctions.costFunction_regularization, CustomFunctions.gradient_regularization)

        test_X = calculateX(X_test, indexes)
        if np.sqrt(sum(np.power(test_X.dot(theta) - y_test, 2))/len(y_test)) < 23.0:
            changing_theta = changing_theta + theta

            num += 1
            

            IO.printError(test_X, y_test, theta)

    # ----------------- TEST

    changing_theta = changing_theta / num
    learning_X = calculateX(characteristics, indexes)
    IO.printError(learning_X, intensity, changing_theta)

    # ----------------------

    test_characteristics = IO.readData("test.tab")
    test_X = calculateX(test_characteristics, indexes)
    predictResults(changing_theta, test_X, 'result')


def calculateX(characteristics, indexes):
...
def learnTheta(X, intensity, costFun=CustomFunctions.costFunction, gradientFun=CustomFunctions.gradient):
